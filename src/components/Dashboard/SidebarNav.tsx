...
const SidebarNav: React.FC<SidebarNavProps> = ({ className }) => {
  return (
    <aside className={cn('fixed top-0 left-0 h-screen w-64 bg-sidebar text-sidebar-foreground flex flex-col', className)}>
      <div className="p-4 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Box size={28} className="text-primary" />
          <span className="font-semibold text-lg">bo</span>
        </div>
        <Button variant="ghost" size="icon" className="md:hidden"> {/* Hamburger for mobile, if needed */}
          <Menu size={20} />
        </Button>
      </div>
        ...
        {mainNavItems.map((item) => (
          <Button
            ...
            className={cn(
              ...
              item.isActive ? 'bg-primary/10 text-primary hover:bg-primary/20' : 'hover:bg-muted/50'
            )}
            asChild
          >
            <a href={item.href}>
